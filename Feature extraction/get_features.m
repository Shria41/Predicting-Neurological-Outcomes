...
if(cpc==1 || cpc==2)
    ...
    recording_features=[signal_mean signal_std alpha_psd_mean beta_psd_mean ...
    quality_score];
end
%disp(recording_features)
%disp(length(signal_mean));
%disp(length(signal_std));
%disp(length(delta_psd_mean));
%disp(length(theta_psd_mean));
%disp("Signal mean");
%disp(signal_mean)

% Combine the features from the patient metadata and the recording data and metadata.
features=[patient_features recording_features cpc];
